# Tinox Compiler Bugs — Archiv (gefixt/abgeschlossen)

Vollständiges historisches Log aller bisher gefundenen und gefixten Bugs
sowie abgeschlossener Feature-Implementierungen, ursprünglich beim Portieren
von jq nach Tinox (`/home/tg7c49/git/jgrep-tinox`) begonnen. Jeder Eintrag
enthält Root Cause, Fix und Verifikation — bewusst NICHT gelöscht, weil die
Begründungen (verworfene Ansätze, Design-Entscheidungen, Querverweise
zwischen Bugs) für künftige Arbeit relevant bleiben.

**Für aktuell offene Punkte siehe `bugs.md`** — dort steht nur, was noch
nicht erledigt ist; alles hier ist abgeschlossen. Einträge sind
chronologisch, spätere Bugs bauen teils auf früheren Fixes auf.
...
`a()+b()`) bräuchte Post-Call-Checks oder setjmp. (Der zweite hier ursprünglich
genannte Punkt — `try`-`finally` ohne `catch` schluckt die Exception — wurde
GEFIXT in [[Bug 42]], direkt im Anschluss an diesen Eintrag.)
